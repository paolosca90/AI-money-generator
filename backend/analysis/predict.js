...
import { TRADING_STRATEGIES, calculateStrategyTargets, getOptimalStrategy, getStrategyRecommendation, calculatePositionSize } from "./trading-strategies";
// Generates AI-powered trading predictions with multiple strategy options.
export const predict = api({ expose: true, method: "POST", path: "/analysis/predict" }, async (req) => {
    const { symbol, strategy: userStrategy, riskPercentage = 2, accountBalance = 10000 } = req;
    ...
    try {
        ...
        if (availableTimeframes.length === 0) {
            ...
            throw APIError.unavailable(`Unable to fetch market data for ${symbol}. The system will continue to work with simulated data for demonstration purposes.`);
        }
        ...
        if (marketData["1m"])
            completeMarketData["1m"] = marketData["1m"];
        if (marketData["1h"])
            completeMarketData["1h"] = marketData["1h"];
        ...
        const priceTargets = calculateStrategyTargets(optimalStrategy, currentPrice, atr, aiAnalysis.direction, symbol);
        // Calculate recommended position size
        const recommendedLotSize = calculatePositionSize(optimalStrategy, accountBalance, riskPercentage, priceTargets.riskAmount);
        ...
        throw APIError.internal(`Failed to generate trading signal for ${symbol}. This could be due to MT5 connection issues or temporary service unavailability.`);
    }
});
function determineRiskLevel(strategy, aiAnalysis, marketData) {
    ...
    if (volatility > strategyConfig.volatilityThreshold * 1.5)
        riskScore += 1;
    if (volatility < strategyConfig.volatilityThreshold * 0.5)
        riskScore -= 1;
    // Adjust based on market structure
    if (aiAnalysis.priceAction.structure === "NEUTRAL")
        riskScore += 1;
