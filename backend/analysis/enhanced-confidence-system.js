 ...
 */
export function calculateEnhancedConfidence(indicators5m, indicators15m, indicators30m, multiTimeframeAnalysis, marketContext, direction, symbol, historicalWinRate, institutionalAnalysis) {
    const factors = {
 ...
 */
function calculateTechnicalAlignment(indicators5m, indicators15m, indicators30m, direction) {
    ...
    macds.forEach(macd => {
        if (direction === "LONG") {
            if (macd.line > macd.signal && macd.histogram > 0)
                alignmentScore += 15 * macdWeight;
            else if (macd.line > macd.signal)
                alignmentScore += 10 * macdWeight;
            else if (macd.histogram > 0)
                alignmentScore += 5 * macdWeight;
        }
        else {
            if (macd.line < macd.signal && macd.histogram < 0)
                alignmentScore += 15 * macdWeight;
    ...
    smaAlignments.forEach(sma => {
        if (direction === "LONG") {
            if (sma.sma20 > sma.sma50 && sma.sma50 > sma.sma200)
                alignmentScore += 20 * maWeight;
            else if (sma.sma20 > sma.sma50)
                alignmentScore += 10 * maWeight;
        }
        else {
            if (sma.sma20 < sma.sma50 && sma.sma50 < sma.sma200)
                alignmentScore += 20 * maWeight;
 ...
 */
function calculateVolumeConfirmation(indicators5m, indicators15m, indicators30m) {
    ...
    const momentumAlignment = [momentum5m, momentum15m, momentum30m].filter(m => Math.sign(momentum5m) === Math.sign(m)).length;
 ...
 */
function calculateMarketConditionsScore(marketContext, multiTimeframeAnalysis) {
 ...
 */
function calculateHistoricalPerformanceScore(historicalWinRate, symbol) {
    ...
    if (historicalWinRate > 0.5)
        return 70;
    if (historicalWinRate > 0.4)
        return 60;
    if (historicalWinRate > 0.3)
        return 50;
    return 30;
}
/**
 ...
 */
function calculateRiskAdjustmentScore(multiTimeframeAnalysis, marketContext) {
 ...
 */
function calculateMomentumStrength(indicators5m, indicators15m, direction) {
    ...
    if (direction === "LONG") {
        if (roc5m > 0 && roc15m > 0)
            score += 40;
        else if (roc5m > 0 || roc15m > 0)
            score += 20;
        // Bonus for accelerating momentum
        if (roc5m > roc15m && roc5m > 0)
            score += 20;
    }
    else {
        if (roc5m < 0 && roc15m < 0)
    ...
    if (direction === "LONG" && macd5m.histogram > 0)
        score += 20;
    if (direction === "SHORT" && macd5m.histogram < 0)
        score += 20;
 ...
 */
function calculateVolatilityFilterScore(multiTimeframeAnalysis, marketContext) {
    let score = 50; // Base score
 ...
 * Apply dynamic thresholds based on market conditions
 */
function applyDynamicThresholds(baseConfidence, marketContext, multiTimeframeAnalysis) {
 ...
 */
function generateWarnings(factors, marketContext, multiTimeframeAnalysis) {
    const warnings = [];
 ...
 */
function calculateInstitutionalAlignment(institutionalAnalysis, direction) {
    ...
    const hasHighVolatilitySession = institutionalAnalysis.activeSessions.some(session => session.volatilityMultiplier >= 1.2);
 ...
 */
function calculateOrderBlockConfirmation(institutionalAnalysis, direction) {
    if (!institutionalAnalysis)
        return 30; // Low if no analysis
    const { orderBlocks } = institutionalAnalysis;
    if (orderBlocks.length === 0)
        return 20;
    // Filter order blocks by direction
    const relevantOBs = orderBlocks.filter(ob => (direction === "LONG" && ob.type === "BULLISH") ||
 ...
 */
function calculateLiquidityZoneConfirmation(institutionalAnalysis, direction) {
    if (!institutionalAnalysis)
        return 30; // Low if no analysis
    ...
    const relevantZones = supplyDemandZones.filter(zone => (direction === "LONG" && zone.type === "DEMAND") ||
 ...
 */
function calculateMarketMakerConfidence(institutionalAnalysis) {
    if (!institutionalAnalysis)
        return 40; // Neutral if no analysis
 ...
 */
function calculateInstitutionalScore(factors) {
 ...
 */
function generateRecommendations(confidence, factors, marketContext, institutionalAnalysis) {
    ...
    const institutionalScore = calculateInstitutionalScore(factors);
    if (confidence >= 85 && institutionalScore >= 80)
        suggestedLotSizeMultiplier = 2.0; // Max size
    else if (confidence >= 75 && institutionalScore >= 70)
        suggestedLotSizeMultiplier = 1.5;
    else if (confidence >= 65 && institutionalScore >= 60)
        suggestedLotSizeMultiplier = 1.2;
    else if (confidence >= 55 && institutionalScore >= 50)
        suggestedLotSizeMultiplier = 1.0;
    else if (confidence >= 45 && institutionalScore >= 40)
        suggestedLotSizeMultiplier = 0.5;
    else
        suggestedLotSizeMultiplier = 0.1;
    // Risk adjustment with institutional considerations
    let riskAdjustment;
    if (factors.riskAdjustment >= 80 && institutionalScore >= 70)
        riskAdjustment = "INCREASE";
    else if (factors.riskAdjustment >= 60 && institutionalScore >= 50)
        riskAdjustment = "NORMAL";
    else
        riskAdjustment = "REDUCE";
    // Timeframe recommendation with institutional timing
    let timeframeRecommendation;
