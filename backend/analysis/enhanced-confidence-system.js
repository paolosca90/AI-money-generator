 ...
 */
export function calculateEnhancedConfidence(indicators5m, indicators15m, indicators30m, multiTimeframeAnalysis, marketContext, direction, symbol, historicalWinRate) {
    const factors = {
 ...
 */
function generateRecommendations(confidence, factors, marketContext) {
    ...
    let riskAdjustment;
    if (factors.riskAdjustment >= 80)
        riskAdjustment = "INCREASE";
    else if (factors.riskAdjustment >= 60)
        riskAdjustment = "NORMAL";
    else
        riskAdjustment = "REDUCE";
    // Timeframe recommendation
    let timeframeRecommendation;
    if (factors.momentumStrength >= 80)
        timeframeRecommendation = "SHORT_TERM";
    else if (factors.multiTimeframeConfluence >= 75)
