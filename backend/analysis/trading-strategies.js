export var TradingStrategy;
(function (TradingStrategy) {
    TradingStrategy["SCALPING"] = "SCALPING";
    TradingStrategy["INTRADAY"] = "INTRADAY";
...
export function calculateStrategyTargets(strategy, currentPrice, atr, direction, symbol, spread) {
    ...
    const stopLossDistance = Math.max(adjustedATR * config.stopLossMultiplier, spread * 3);
...
export function getOptimalStrategy(marketData, symbol, userPreference) {
...
function isStrategyValid(strategy, marketData, symbol) {
    const config = TRADING_STRATEGIES[strategy];
    const volatility = calculateMarketVolatility(marketData);
    const trendStrength = calculateTrendStrength(marketData);
    return (volatility <= config.volatilityThreshold * 2 &&
        trendStrength >= config.trendStrengthRequired * 0.8);
}
function calculateStrategyScore(strategy, volatility, trendStrength) {
    const config = TRADING_STRATEGIES[strategy];
    let score = 50;
...
function calculateTrendStrength(marketData) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
...
function getSymbolCharacteristics(symbol) {
...
export function getStrategyRecommendation(strategy, marketData, aiAnalysis) {
    const config = TRADING_STRATEGIES[strategy];
    const volatility = calculateMarketVolatility(marketData);
    const trendStrength = calculateTrendStrength(marketData);
    ...
    if (aiAnalysis.institutionalAnalysis) {
        ...
        if (institutional.orderBlocks.length > 0) {
            const bullishOBs = institutional.orderBlocks.filter((ob) => ob.type === "BULLISH").length;
            const bearishOBs = institutional.orderBlocks.filter((ob) => ob.type === "BEARISH").length;
        ...
        if (institutional.supplyDemandZones.length > 0) {
            const freshZones = institutional.supplyDemandZones.filter((zone) => zone.status === "FRESH").length;
            recommendation += `• Zone S/D Fresche: ${freshZones}\n`;
        }
        // Active Sessions
        if (institutional.activeSessions.length > 0) {
            const sessionNames = institutional.activeSessions.map((s) => s.name).join(", ");
            recommendation += `• Sessioni Attive: ${sessionNames}\n`;
        }
        // Kill Zones
        const activeKillZone = institutional.killZones.find((kz) => kz.isActive);
        if (activeKillZone) {
...
export function calculatePositionSize(strategy, accountBalance, riskPercentage, riskAmount) {
    const config = TRADING_STRATEGIES[strategy];
...
//# sourceMappingURL=trading-strategies.js.map