...
export function calculateStrategyTargets(strategy, currentPrice, atr, direction, symbol) {
...
export function getOptimalStrategy(marketData, aiAnalysis, symbol, userPreference) {
    ...
    return Object.entries(scores).reduce((best, [strategy, score]) => score > scores[best] ? strategy : best, "INTRADAY");
}
function isStrategyValid(strategy, marketData, aiAnalysis, symbol) {
    const config = TRADING_STRATEGIES[strategy];
    const volatility = calculateMarketVolatility(marketData);
    const trendStrength = calculateTrendStrength(marketData);
    // Check if conditions meet strategy requirements
    return (aiAnalysis.confidence >= config.minConfidence &&
...
function calculateStrategyScore(strategy, volatility, trendStrength, confidence) {
    const config = TRADING_STRATEGIES[strategy];
    let score = 0;
