...
export const getTopSignals = api({
    method: "GET",
    ...
    `;
      ...
      `;
        ...
        const autoSignals = topSignals.slice(0, 3).map(signal => transformToAutoSignal(signal));
...
function transformToAutoSignal(signal) {
...
function getTimeframeFromStrategy(strategy) {
    switch (strategy) {
        case 'SCALPING':
            return '5m';
        case 'INTRADAY':
...
export const getSignalStats = api({
    expose: true,
    method: "GET",
...
export const forceSignalGeneration = api({
    expose: true,
    method: "POST",
...
//# sourceMappingURL=top-signals.js.map