...
import { TradingStrategy, TRADING_STRATEGIES, calculateStrategyTargets, getOptimalStrategy, getStrategyRecommendation, calculatePositionSize } from "./trading-strategies";
export async function generateSignalForSymbol(symbol, mt5Config, tradeParams, userStrategy, requireRealData = false) {
    ...
    const priceTargets = calculateStrategyTargets(optimalStrategy, currentPrice, atr, aiAnalysis.direction, symbol, spread);
...
function calculateExpirationTime(strategy, maxHoldingHours) {
    const now = new Date();
