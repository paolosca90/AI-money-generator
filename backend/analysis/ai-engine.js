...
import { calculateEnhancedIndicators, analyzeMultiTimeframeConfluence, getMarketConditionContext } from "./enhanced-technical-analysis";
import { calculateEnhancedConfidence } from "./enhanced-confidence-system";
import { performInstitutionalAnalysis } from "./institutional-analysis";
import { learningEngine } from "../ml/learning-engine";
const geminiApiKey = secret("GeminiApiKey");
// Cache for Gemini responses to reduce API calls
const geminiCache = new Map();
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
export async function analyzeWithAI(marketData, symbol, strategy) {
    ...
    const indicators5m = calculateEnhancedIndicators([data5m.open], [data5m.high], [data5m.low], [data5m.close]);
    const indicators15m = calculateEnhancedIndicators([data15m.open], [data15m.high], [data15m.low], [data15m.close]);
    const indicators30m = calculateEnhancedIndicators([data30m.open], [data30m.high], [data30m.low], [data30m.close]);
    ...
    const traditionalDirection = determineDirection(priceActionAnalysis, smartMoneyAnalysis, volumeAnalysis, geminiAnalysis);
    // Use enhanced direction determination with technical confluence
    const enhancedDirection = determineEnhancedDirection(indicators5m, indicators15m, indicators30m, multiTimeframeAnalysis, traditionalDirection);
    ...
    const institutionalAnalysis = performInstitutionalAnalysis(data5m, data15m, data30m, data30m, data30m, data30m, symbol);
    ...
    const traditionalConfidence = calculateConfidence(priceActionAnalysis, smartMoneyAnalysis, volumeAnalysis, professionalAnalysis, geminiAnalysis);
    // Calculate enhanced confidence with sophisticated scoring
    const enhancedConfidence = calculateEnhancedConfidence(indicators5m, indicators15m, indicators30m, multiTimeframeAnalysis, marketContext, enhancedDirection, symbol, undefined, // historicalWinRate
...
function analyzePriceActionEnhanced(data5m, data15m, data30m, symbol) {
...
function calculateTrendStrength(prices) {
    ...
    const priceChanges = prices.map((p, i) => i > 0 ? (p.close - prices[i - 1].close) / prices[i - 1].close : 0).slice(1);
...
function determineTrendDirection(prices, trendStrength) {
    if (Math.abs(trendStrength) < 0.3)
        return "SIDEWAYS";
    return trendStrength > 0 ? "UPTREND" : "DOWNTREND";
}
function analyzeStructureBreakEnhanced(prices, symbol) {
    ...
    if (highBreak > threshold)
        return 1; // Strong bullish break
    if (lowBreak > threshold)
        return -1; // Strong bearish break
    // Partial breaks
    if (highBreak > threshold * 0.5)
        return 0.5;
    if (lowBreak > threshold * 0.5)
        return -0.5;
    return 0; // No significant break
}
function calculateEnhancedSwingLevels(prices, symbol) {
...
function calculateEnhancedBreakoutProbability(prices, symbol) {
...
function calculateTimeCompression(prices) {
...
function getSymbolVolatilityExpectation(symbol) {
...
function analyzeSmartMoneyEnhanced(data5m, data15m, data30m, symbol) {
...
function analyzeInstitutionalFlowEnhanced(volumes, prices, symbol) {
    ...
    if (volumes[0] < avgVolume * threshold)
        return "NEUTRAL";
    const priceDeviation = (currentPrice - volumeWeightedPrice) / volumeWeightedPrice;
    if (priceDeviation > 0.003)
        return "BUYING"; // 0.3% above VWAP with high volume
    if (priceDeviation < -0.003)
        return "SELLING"; // 0.3% below VWAP with high volume
    return "NEUTRAL";
}
function analyzeVolumeProfilePatternEnhanced(volumes, prices, symbol) {
    ...
    if (maxScore === accumulationScore && accumulationScore > 0)
        return "ACCUMULATION";
    if (maxScore === distributionScore && distributionScore > 0)
        return "DISTRIBUTION";
    return "CONSOLIDATION";
}
function analyzeOrderFlowEnhanced(volumes, prices, symbol) {
...
function identifyLiquidityZonesEnhanced(data5m, data15m, data30m, symbol) {
    const currentPrice = data5m.close;
    const levels = [];
    // Previous highs and lows with enhanced weighting
    levels.push({ price: data30m.high, weight: 3 }, { price: data30m.low, weight: 3 }, { price: data15m.high, weight: 2 }, { price: data15m.low, weight: 2 }, { price: data5m.high, weight: 1 }, { price: data5m.low, weight: 1 });
...
function calculatePsychologicalLevels(currentPrice, symbol) {
    const levels = [];
    ...
    spacings.forEach((spacing) => {
        ...
        levels.push(nearestLevel, nearestLevel + spacing, nearestLevel - spacing, nearestLevel + (spacing * 2), nearestLevel - (spacing * 2));
    });
    return levels.filter(level => level > 0);
}
function calculateEnhancedSupportResistance(data5m, data15m, data30m, symbol) {
    const currentPrice = data5m.close;
...
function analyzeVolumeProfile(data5m, data15m, data30m) {
...
async function analyzeProfessionalTraders(symbol, marketData) {
...
function getTopTradersForAsset(symbol) {
...
function analyzeTraderConsensus(symbol, marketData) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
...
function calculateProfessionalRiskReward(marketData) {
    const data5m = marketData["5m"];
...
function determineOptimalTimeframe(symbol) {
    const timeframeMap = {
...
async function analyzeWithGeminiCached(marketData, symbol, additionalData) {
    try {
...
function createAdvancedTradingPrompt(marketData, symbol, additionalData) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
...
function parseGeminiResponse(text) {
    try {
        ...
        const direction = directionMatch?.[1]?.toUpperCase() || "LONG";
...
function enhancedFallbackAnalysis(marketData, additionalData) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
    ...
    priceChanges.forEach(change => {
        if (change > 0.001)
            bullishSignals += 3; // 0.1% threshold
    ...
    if (data5m.indicators.rsi < 30)
        bullishSignals += 2; // Oversold
    else if (data5m.indicators.rsi > 70)
        bearishSignals += 2; // Overbought
    totalWeight += 2;
    if (data5m.indicators.macd > 0)
        bullishSignals += 1;
    else
        bearishSignals += 1;
    totalWeight += 1;
    // Smart money analysis (weight: 4)
    if (additionalData.smartMoney.institutionalFlow === "BUYING")
        bullishSignals += 2;
    else if (additionalData.smartMoney.institutionalFlow === "SELLING")
        bearishSignals += 2;
    totalWeight += 2;
    if (additionalData.smartMoney.volumeProfile === "ACCUMULATION")
        bullishSignals += 1;
    else if (additionalData.smartMoney.volumeProfile === "DISTRIBUTION")
        bearishSignals += 1;
    totalWeight += 1;
    if (additionalData.smartMoney.orderFlow === "BULLISH")
        bullishSignals += 1;
    else if (additionalData.smartMoney.orderFlow === "BEARISH")
        bearishSignals += 1;
    totalWeight += 1;
    // Price action analysis (weight: 3)
    if (additionalData.priceAction.trend === "UPTREND")
        bullishSignals += 2;
    else if (additionalData.priceAction.trend === "DOWNTREND")
        bearishSignals += 2;
    totalWeight += 2;
    if (additionalData.priceAction.structure === "BULLISH")
        bullishSignals += 1;
    else if (additionalData.priceAction.structure === "BEARISH")
        bearishSignals += 1;
    totalWeight += 1;
...
function fallbackAnalysis(marketData) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
    ...
    let bearishSignals = 0;
    // Price momentum
    if (data5m.close > data15m.close)
        bullishSignals++;
    else
        bearishSignals++;
    if (data15m.close > data30m.close)
        bullishSignals++;
    else
        bearishSignals++;
    // Volume confirmation
    if (data5m.volume > data15m.volume && data5m.close > data5m.open)
        bullishSignals++;
    if (data5m.volume > data15m.volume && data5m.close < data5m.open)
        bearishSignals++;
...
function analyzeVolumeDistribution(timeframes) {
    const volumes = timeframes.map(tf => tf.volume);
    const avgVolume = volumes.reduce((sum, vol) => sum + vol, 0) / volumes.length;
    if (volumes[0] > avgVolume * 1.5)
        return "HIGH_VOLUME";
    if (volumes[0] < avgVolume * 0.5)
        return "LOW_VOLUME";
    return "NORMAL_VOLUME";
}
function identifyVolumeNodes(timeframes) {
...
function determineDirection(priceAction, smartMoney, volumeAnalysis, geminiAnalysis) {
    let bullishScore = 0;
    let bearishScore = 0;
    // Price action weight: 30%
    if (priceAction.trend === "UPTREND")
        bullishScore += 0.15;
    if (priceAction.trend === "DOWNTREND")
        bearishScore += 0.15;
    if (priceAction.structure === "BULLISH")
        bullishScore += 0.15;
    if (priceAction.structure === "BEARISH")
        bearishScore += 0.15;
    // Smart money weight: 40%
    if (smartMoney.institutionalFlow === "BUYING")
        bullishScore += 0.15;
    if (smartMoney.institutionalFlow === "SELLING")
        bearishScore += 0.15;
    if (smartMoney.volumeProfile === "ACCUMULATION")
        bullishScore += 0.1;
    if (smartMoney.volumeProfile === "DISTRIBUTION")
        bearishScore += 0.1;
    if (smartMoney.orderFlow === "BULLISH")
        bullishScore += 0.15;
    if (smartMoney.orderFlow === "BEARISH")
        bearishScore += 0.15;
    // Gemini AI weight: 30%
    if (geminiAnalysis.direction === "LONG")
        bullishScore += 0.3;
    if (geminiAnalysis.direction === "SHORT")
        bearishScore += 0.3;
    return bullishScore > bearishScore ? "LONG" : "SHORT";
}
function calculateConfidence(priceAction, smartMoney, volumeAnalysis, professionalAnalysis, geminiAnalysis) {
 ...
 */
function determineEnhancedDirection(indicators5m, indicators15m, indicators30m, multiTimeframeAnalysis, traditionalDirection) {
    let bullishScore = 0;
    let bearishScore = 0;
    ...
    rsiValues.forEach((rsi, index) => {
        ...
        if (rsi < 30)
            bullishScore += 2 * weight; // Oversold
        else if (rsi < 50)
            bullishScore += 1 * weight; // Bearish but not extreme
        else if (rsi > 70)
            bearishScore += 2 * weight; // Overbought
        else if (rsi > 50)
            bearishScore += 1 * weight; // Bullish but not extreme
    });
    ...
    macdValues.forEach((macd, index) => {
        ...
        if (macd.line > macd.signal) {
            ...
            if (macd.histogram > 0)
                bullishScore += 1 * weight; // Increasing momentum
        }
        else {
            bearishScore += 2 * weight;
            if (macd.histogram < 0)
                bearishScore += 1 * weight; // Increasing momentum
        }
    });
