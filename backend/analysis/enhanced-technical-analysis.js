 ...
 */
export function calculateEnhancedRSI(prices, period = 14) {
 ...
 */
export function calculateEnhancedMACD(prices) {
    if (prices.length < 26) {
 ...
 */
export function calculateEnhancedATR(highs, lows, closes, period = 14) {
 ...
 */
export function analyzeMultiTimeframeConfluence(tf5m, tf15m, tf30m) {
    ...
    if (rsiBullish === 3)
        trendScore += 2;
    else if (rsiBullish === 2)
        trendScore += 1;
    else if (rsiBearish === 3)
        trendScore -= 2;
    else if (rsiBearish === 2)
        trendScore -= 1;
    // Volume confluence
    ...
    const confluenceScore = Math.min(100, Math.max(0, 50 + (trendScore * 10) + (momentumAlignment * 5) + (volumeConfirmation * 10)));
    ...
    if (confluenceScore >= 80)
        ...
        trendAlignment = "STRONG_BEAR";
    else if (confluenceScore <= 40)
        trendAlignment = "BEAR";
    else
        trendAlignment = "NEUTRAL";
    // Momentum alignment analysis
    let momentumAlignmentResult;
    if (Math.abs(momentumAlignment) === 3)
        momentumAlignmentResult = momentumAlignment > 0 ? "INCREASING" : "DECREASING";
    else if (Math.abs(momentumAlignment) >= 1)
        momentumAlignmentResult = momentumAlignment > 0 ? "INCREASING" : "DECREASING";
    else
        momentumAlignmentResult = "NEUTRAL";
 ...
 */
export function getMarketConditionContext() {
    const now = new Date();
 ...
 */
export function calculateEnhancedIndicators(opens, highs, lows, closes, volumes) {
...
//# sourceMappingURL=enhanced-technical-analysis.js.map