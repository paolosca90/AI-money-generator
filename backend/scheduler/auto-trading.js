...
async function updateSignalGenerationStats(totalGenerated, topSelected) {
    try {
...
async function simulateTradeExecution(signal) {
    try {
...
async function simulateTradeClose(signal, holdingTimeMs) {
    try {
...
function calculateHoldingTime(strategy, maxHours) {
    const baseTime = {
        'SCALPING': { min: 1, max: 15 }, // 1-15 minuti
        'INTRADAY': { min: 60, max: 360 }, // 1-6 ore
    };
    const times = baseTime[strategy] || { min: 30, max: 180 };
...
function calculateRealisticOutcome(signal) {
...
async function updateMLMetrics(stats) {
    try {
        const totalTrades = Number(stats.total_trades);
        const winningTrades = Number(stats.winning_trades);
