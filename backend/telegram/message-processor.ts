...
async function handlePredictCommand(chatId: number, command: string, userId?: number): Promise<void> {
  ...
  try {
    ...
    const prediction = await analysis.predict({ 
      symbol, 
      strategy,
      riskPercentage: userPrefs?.riskPercentage,
      accountBalance: userPrefs?.accountBalance
    });
...
// New functions for trading mode setup workflow
