...
export const storeVPSConfig = api({ expose: true, method: "POST", path: "/telegram/vps/config" }, async (req) => {
...
export const getVPSStatus = api({ expose: true, method: "GET", path: "/telegram/vps/status/:userId" }, async (req) => {
...
async function configureVPSAsync(userId) {
    try {
...
async function installSoftwareOnVPS(config) {
    try {
...
async function configureMT5OnVPS(config) {
    try {
...
async function startTradingBotOnVPS(config) {
    try {
...
async function sendVPSNotification(userId, message) {
    try {
        // Get user's chat ID
...
export async function handleVPSCommand(chatId, userId, command) {
...
async function handleVPSMainMenu(chatId, userId) {
    const { sendMessage, createInlineKeyboard } = await import("./telegram-client");
    const config = await telegramDB.queryRow `
    SELECT * FROM vps_configurations WHERE user_id = ${userId}
  `;
...
async function handleVPSStatus(chatId, userId) {
    const { sendMessage } = await import("./telegram-client");
    const config = await telegramDB.queryRow `
    SELECT * FROM vps_configurations WHERE user_id = ${userId}
  `;
...
async function handleVPSRestart(chatId, userId) {
    const { sendMessage } = await import("./telegram-client");
    const config = await telegramDB.queryRow `
    SELECT * FROM vps_configurations WHERE user_id = ${userId}
  `;
...
async function handleVPSLogs(chatId, userId) {
    const { sendMessage } = await import("./telegram-client");
    const config = await telegramDB.queryRow `
    SELECT * FROM vps_configurations WHERE user_id = ${userId}
  `;
...
export async function handleVPSSetup(chatId, userId, message) {
    const { sendMessage, createInlineKeyboard } = await import("./telegram-client");
...
export async function handleVPSSetupCallback(chatId, userId, callbackData) {
    const { sendMessage } = await import("./telegram-client");
...
//# sourceMappingURL=vps-manager.js.map