...
export class Client {
    ...
    public readonly telegram: telegram.ServiceClient
    ...
    constructor(target: BaseURL, options?: ClientOptions) {
        ...
        this.telegram = new telegram.ServiceClient(base)
    }
...
export namespace analysis {
    ...
    export class ServiceClient {
         ...
         */
        public async predict(params: RequestType<typeof api_analysis_predict_predict>): Promise<ResponseType<typeof api_analysis_predict_predict>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/analysis/predict`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_analysis_predict_predict>
        }

        /**
         * Records trading results for AI model improvement.
         */
        public async recordFeedback(params: RequestType<typeof api_analysis_feedback_recordFeedback>): Promise<ResponseType<typeof api_analysis_feedback_recordFeedback>> {
            // Now make the actual call to the API
 ...
 */
import {
    getVPSStatus as api_telegram_vps_manager_getVPSStatus,
    storeVPSConfig as api_telegram_vps_manager_storeVPSConfig
} from "~backend/telegram/vps-manager";
import { webhook as api_telegram_webhook_webhook } from "~backend/telegram/webhook";
...
export namespace telegram {
    ...
    export class ServiceClient {
        ...
        constructor(baseClient: BaseClient) {
            this.baseClient = baseClient
            this.getVPSStatus = this.getVPSStatus.bind(this)
            this.storeVPSConfig = this.storeVPSConfig.bind(this)
         ...
         */
        public async getVPSStatus(params: { userId: number }): Promise<ResponseType<typeof api_telegram_vps_manager_getVPSStatus>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/telegram/vps/status/${encodeURIComponent(params.userId)}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_telegram_vps_manager_getVPSStatus>
        }

        /**
         * Store VPS configuration for a user
         */
        public async storeVPSConfig(params: RequestType<typeof api_telegram_vps_manager_storeVPSConfig>): Promise<ResponseType<typeof api_telegram_vps_manager_storeVPSConfig>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/telegram/vps/config`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_telegram_vps_manager_storeVPSConfig>
        }

        /**
         * Handles incoming Telegram webhook updates.
         */
        public async webhook(params: RequestType<typeof api_telegram_webhook_webhook>): Promise<ResponseType<typeof api_telegram_webhook_webhook>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/telegram/webhook`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_telegram_webhook_webhook>
        }
    }
}
